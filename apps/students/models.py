...
class Student(models.Model):
    ...
    current_class = models.ForeignKey(Stud_Class, on_delete=models.SET_NULL, blank=True, null=True)
    date_of_admission = models.DateField(default=timezone.now)
    
    mobile_num_regex = RegexValidator(regex="^[0-9]{10,15}$", message="Enter a valid phone number")
    student_mobile = models.CharField(validators=[mobile_num_regex], max_length=14, blank=False)
    parent_mobile = models.CharField(validators=[mobile_num_regex], max_length=14, blank=True)
